 ...
 *                    the socket number/id of communications port is returned
 *                 otherwise, the program exits, as robot processing cannot continue
 * @note           on Linux and Mac computers, sockets are ints, 
 *                    so rConnect returns an int
 *                 on Windows computers, sockets have type SOCKET,
 *                    so rConnect returns a SOCKET
 *                 together, these are defined as a ROBOT_CONNECTION_TYPE
 *
 * @post           subsequent communications will take place through
 *                 this robot address, unless changed by rSetConnection
 */
 ...
 * @note                  a socket has type int on Linux or Mac; type SOCKET on Windows
 *                        together these are defined as ROBOT_CONNECTION_TYPE
 */
...
/**
 * @brief Turn on and off the printing transmission and echong of Bluetooth transmissions
 ...
 * @param onOff     char 'y' enables printing of Bluetooth communications \n
 *                  char 'n' disables printng                             \n
